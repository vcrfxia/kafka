...
public class StreamsUpgradeTest {
    ...
    public static KafkaStreams buildStreams(final Properties streamsProperties) {
        ...
        if (runTableAgg) {
            ...
            try {
                buildTableAgg(dataStream, aggProducePrefix, expectedAggPrefixes);
            } catch (final Exception e) {
                System.err.println("Caught " + e.getMessage());
            }
        }
    ...
    private static void buildTableAgg(final KStream<String, Integer> sourceStream,
                                      final String aggProducePrefix,
                                      final List<String> expectedAggPrefixes) {
        sourceStream.toTable()
            .mapValues((k, v) -> aggProducePrefix)
            ...
            .aggregate(
                ...
                (k, v, agg) -> {
                    ...
                    final boolean shouldLog = updated || (agg.recordsProcessed % 100 == 0);
                ...
                (k, v, agg) -> agg,
                Materialized.with(stringSerde, new AggSerde()))
            .mapValues((k, vAgg) -> String.join(",", vAgg.seenPrefixes))
            .toStream()
            .to("table-agg-result", Produced.with(stringSerde, stringSerde));
    }
